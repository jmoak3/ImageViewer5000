...
void RenderMesh(int width, int height, Triangle *tris)
{
	...
	Hit hit; Vector2 pt, V0V1, V2V0; int top,bot,lft,rgt;
	...
	for (i=0;i<numTris;++i)
	{
		...
		// DetProduct tests if left or right!
		// @--@  triangles are always clockwise. If  
		// | /    one is counter clockwise, it's backwards,
		...
		for (row=top;row<bot;++row)
		{
			...
			{
				avgZ = (a.z+b.z+c.z)/3.f; //Is this triangle closer than last (if any)?
				if (avgZ < DepthBuffer[row*width + column] && avgZ > 0.f)
				{
					pt.x = column; pt.y = row;
					if (DoesPointLieOnTri(&(tris[i]), &pt, &hit))
					{
						DepthBuffer[row*width + column] = avgZ; // Store the closest Z
					...
					}   // This saves the color - which is all rendering is!
				}
			}
		}
	}
...
void Render(int width, int height, char *fileName, Vector3 cam)
{
	...
	double finalTime = (endTime-startTime)/600.0;
